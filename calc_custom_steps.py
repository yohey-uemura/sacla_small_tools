...
def InsertText():
    ...
    if selected.get() =='Eng':
        ...
        text.insert(END,"########################################################\n")
        text.insert(END,"#     how to write: 'start', 'end', 'step'             #\n")
        text.insert(END,"#             <Example (for PPODL in ps)>              #\n")
        text.insert(END,"#"+f"{'t0 = 100':^54}"+"\n#")
...
def make_params():
    ...
    if selected.get() == 'Eng':
        ...
        if '.csv' in fn:
            ...
            f = open(fn+'.csv','w')
        for v in CC:
            txt+= f'{v/1000:.5f}, {v/1000:.5f}\n'
            f.write(f'{v/1000:.5f}, {v/1000:.5f}\n')
        f.close()
        out.insert(END,"'mono(keV)', 'undl(keV)'\n")
        out.insert(END,txt)
    elif selected.get() == 'Dly':
        for l in tinput:
            if re.match(r'^#.+',l.rstrip()):
                ...
                t0 = float(l.rstrip().split('=')[-1])
            elif re.match(r'^-?\d+.+',l.rstrip()):
                start, end, step = [float(v) for v in l.split(',')]
                i = 0
                if step > 0:
                    while end > start + step*i:
                        params.append(-(start+step*i)+t0)
                        i += 1
                else:
                    while end < start + step*i:
                        params.append(-(start+step*i)+t0)
